...
if (isset($_GET["shop"])) {
    ...
    if ($chargeType == "one-time") {
        $recu = $shopify("POST", "/admin/api/".apiVersion."/application_charges.json", $charge);
        $confirmation_url = $recu["confirmation_url"];
    } else {
        $recu = $shopify("POST", "/admin/api/".apiVersion."/recurring_application_charges.json", $charge);
        $confirmation_url = $recu["confirmation_url"];
    }
    ?>
	<head>
	...
	<div class="container" style="padding:30px 0;margin-top: 50px;border: 5px solid #EC342E;max-width:600px;text-align:center;">
		<img src="https://cdn.shopify.com/s/files/applications/e03e0948951b94a7b424d1a55634d891_512x512.png" style="max-width: 150px;">
